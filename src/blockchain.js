              ...
              self.validateChain().then(errors => {
                ...
                self.getBlockByHeight(self.height)
                  .then(previousBlock => {
                    ...
                    block.hash = SHA256(Buffer(JSON.stringify({...block})).toString()).toString();
    ...
    validateChain() {
      ...
      return new Promise(async (resolve, reject) => {
        const validateBlock = (block) => {
          self.getBlockByHash(block.previousBlockHash).then(b => {
            if(!b){
              resolve(errorLog);
              return;
            }
            b.validate().then(isValid => {
              !isValid ? errorLog.push({[b.height]: 'invalid'}) : null;
              validateBlock(b);
            });
          });
        };
        ...
        self.getBlockByHeight(self.height).then(block => {
          block.validate().then(isvalid => {
            if(!isvalid){
              errorLog.push({[self.height]: 'invalid'});
            }
            validateBlock(block);
          });
        });
